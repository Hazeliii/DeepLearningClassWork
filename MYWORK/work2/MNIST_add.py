...
def PreparePlusData():
    ...
    train_datasets =tf.data.Dataset.zip((tf.data.Dataset.from_tensor_slices((x_train_l, x_train_r)), tf.data.Dataset.from_tensor_slices(y_train))).batch(64)
    print(train_datasets)

    test_datasets =tf.data.Dataset.zip((tf.data.Dataset.from_tensor_slices((x_test_l, x_test_r)), tf.data.Dataset.from_tensor_slices(y_test))).batch(64)
    print(test_datasets)
    # WORK1: ---------------END--------------------
...
class BiasPlusLayer(keras.layers.Layer):
    ...
    def call(self, input):
        return self.bias + input[0] + input[1]
    '''
    def __init__(self, num_outputs, **kwargs):
        super(BiasPlusLayer, self).__init__(**kwargs)
        self.num_outputs = num_outputs
...
def BuildModel():
    ...
    shared_base = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(196,), name='dense_input'),
    ...
    output = Dense(num_classes, activation = 'softmax')(b)
...
def test_fun():
    ...
    # 其他训练参数不用变
    siamese_net.compile(loss=tf.keras.losses.sparse_categorical_crossentropy, optimizer=tf.keras.optimizers.RMSprop(learning_rate=0.001), metrics=['accuracy'])
...
test_img = mpimg.imread('./siamese_net.png') 
